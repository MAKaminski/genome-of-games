...
<div class="srch"><input id="sq" type="search" placeholder="Search 1,181 entries…" autocomplete="off" aria-label="Search"><div id="sres"></div></div>
</div></header><main>`;
}
...
<div><h4>Games by decade</h4>${decs}<a href="/games/">All 619 games</a></div>
<div><h4>Eras</h4>${eras}</div>
<div><h4>The project</h4>
  <a href="/">Home</a><a href="/graph/">Interactive graph</a><a href="/features/">All 168 mechanics</a>
  <a href="/studios/">All 394 studios</a><a href="/methodology/">Methodology &amp; limits</a>
...
168 mechanics, 619 games, 394 companies, 4,371 recorded links. Attribution of "firsts" in games history is frequently
...
function studioCard(id) {
  ...
  <div class="t">${esc(n.n)}</div>
  <div class="d">${esc(n.d || '')}</div>
  <div class="d" style="margin-top:6px;color:var(--dim2)">${n.nt} title${n.nt === 1 ? '' : 's'} in the dataset</div>
</a>`;
}
function cardOf(id) {
  const t = T.N[id].t;
...
function externals(name, kind) {
  const links = [[`Wikipedia`, T.wiki(name)]];
  if (kind !== 'F') links.push([`MobyGames`, T.moby(name)]);
  links.push([`Giant Bomb`, T.giant(name)]);
