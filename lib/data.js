...
const GITHUB = process.env.GITHUB_URL || 'https://github.com/MAKaminski/genome-of-games';

/* Values baked into the page at build time. Only ever publishable keys here —
   the Supabase anon key is safe in the browser because RLS is what protects
   the data, and the PostHog project key is public by design. */
const envTrim = (n, d = '') => (process.env[n] || d).trim();
const PUBLIC = {
  site: SITE,
  supabaseUrl: envTrim('SUPABASE_URL'),
  supabaseKey: envTrim('SUPABASE_ANON_KEY'),
  posthogKey: envTrim('POSTHOG_KEY'),
  posthogHost: envTrim('POSTHOG_HOST', 'https://us.i.posthog.com')
};
...
module.exports = {
  ...
  esc, attr, enc, wiki, moby, giant, SITE, GITHUB, PUBLIC
};
