...
/* Mint a magic-link token for an address with the admin API, creating the
   user when the address is new. Returns the hashed token: the browser hands
   it to POST /auth/v1/verify (see static/gog.js) and gets a session back, so
   no Supabase-hosted redirect page and no allow-listed URL is involved. */
export async function adminGenerateLink(email) {
  const res = await fetch(`${SUPABASE_URL}/auth/v1/admin/generate_link`, {
    method: 'POST',
    headers: { apikey: SERVICE_KEY, authorization: `Bearer ${SERVICE_KEY}`, 'content-type': 'application/json' },
    body: JSON.stringify({ type: 'magiclink', email, options: { redirect_to: `${SITE}/newsletter/` } })
  });
  const out = await res.json().catch(() => ({}));
  if (!res.ok) throw new Error(`Supabase ${res.status}: ${out.msg || out.message || out.error_description || JSON.stringify(out)}`);
  if (!out.hashed_token) throw new Error('generate_link returned no hashed_token');
  return out.hashed_token;
}
